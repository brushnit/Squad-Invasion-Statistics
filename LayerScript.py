...
csv_file_path  = 'layerstatistics.csv' #new csv file

#set int v5FirstMatchId and int v6FirstMatchId to the match IDs of the first live game of each update in json_file_path
v5FirstMatchId = 933
v6FirstMatchId = 4710

#change original_csv_file name to 'invasionlayersSE.csv' for Squad Enhanced analysis
original_csv_file = 'invasionlayers.csv'
...
with open(original_csv_file, newline='') as incsv:
...
class Game:
    def __init__(self, game):
        ...
        self.winnerTickets = game['winnerTickets'] #required
        self.loserTeam = game['loserTeam'] #required
        self.loserFaction = game['loserFaction']
        self.loserTickets = game['loserTickets'] #required
        ...
        self.V5 = v5FirstMatchId <= game['id'] < v6FirstMatchId #filter for matches in v5
        self.preV6 = game['id'] < v6FirstMatchId #filter for all matches before v6
...
def main():
    ...
    newFields = ['attackWin', 'defendWin', 'perAttackWin', 'perDefendWin', 'numMatches'] #fields to be added to csv/columns to be added to dataframe
    ...
    with open(json_file_path, 'r') as json_file:
        ...
        for index, row in df.iterrows():
            ...
            for match in matches['DBLog_Games']:
                ...
                #filter block    
                if game.layer == row['layer'] and game.isLiveMatch: #add more filters using 'and' statements, i.e: and (game.V5 or game.preV5):
